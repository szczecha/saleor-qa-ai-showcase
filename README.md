...
GitHub Actions runs layers in order: API → UI → A11y. API (P0) failures block the build; UI and A11y failures are informational at POC stage. Playwright browser binaries are cached at `~/.cache/ms-playwright` to avoid re-downloading on every run (~300 MB).
