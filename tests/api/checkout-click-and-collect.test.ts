...
test.describe.serial('Checkout — anonymous buyer, click & collect (USD)', () => {
  ...
  // 2. Buyer adds a product — response exposes available collection points
  test('step 2 — checkoutLinesAdd exposes collection points', async () => {
    ...
    const collectionPoint = checkout.availableCollectionPoints.find(
      (w) =>
        (w.clickAndCollectOption as WarehouseClickAndCollectOptionEnum) !== 'DISABLED'
    );
    expect(
      collectionPoint,
      `No collection point available. Points: ${JSON.stringify(checkout.availableCollectionPoints)}`
    ).toBeDefined();
  ...
  // 4. Buyer enters billing address (no shipping address needed for C&C)
  test('step 4 — checkoutBillingAddressUpdate sets billing address', async () => {
    const data =
      await gqlClient.request<CheckoutBillingAddressUpdateResponse>(
        CHECKOUT_BILLING_ADDRESS_UPDATE,
        { id: checkoutId, billingAddress: BILLING_ADDRESS }
      );
  ...
  // 5. Buyer selects a collection point warehouse as the delivery method
  test('step 5 — checkoutDeliveryMethodUpdate selects collection point', async () => {
    const data =
      await gqlClient.request<CheckoutDeliveryMethodUpdateResponse>(
        CHECKOUT_DELIVERY_METHOD_UPDATE,
        { id: checkoutId, deliveryMethodId: collectionPointId }
      );
    ...
    // C&C has no shipping cost
    expect(checkout.shippingPrice.gross.amount).toBe(0);

    orderTotal = checkout.totalPrice.gross.amount;
    expect(orderTotal).toBeGreaterThan(0);
  });

  // 6. Buyer pays via the dummy payment gateway
  test('step 6 — transactionInitialize charges via dummy gateway', async () => {
  ...
  // 7. Checkout completes — order created with zero shipping cost
  test('step 7 — checkoutComplete creates an order', async () => {
    ...
    expect(order?.total?.gross.amount).toBe(orderTotal);
